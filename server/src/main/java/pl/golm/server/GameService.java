...
//            }
    ...
    private void communicateWithClients() throws Exception // according to the game phase this functions executes just one of the blocks
    {
        ...
        if (game.getGameState().equals(GameState.COUNTING_TERRITORIES)) // in fact it is counting territories and dead groups in the same time
        {
            ...
            GameUtils.getDeadGroups(game.getBoard(), game.getPlayer2()).forEach((Field field) -> {try {client1Settings.getBufferedWriter().println(Integer.toString(field.getRow()) + "," + Integer.toString(field.getColumn())); client1Settings.getBufferedWriter().flush();} catch (Exception Exception){Exception.printStackTrace();}});
            ...
            else if (line.equals("Dead groups"))
            {
                ...
                else // white agreed on his white groups
                {
                    ...
                    client2Settings.getBufferedWriter().flush();
                    GameUtils.getDeadGroups(game.getBoard(), game.getPlayer1()).forEach((Field field) -> {try {client2Settings.getBufferedWriter().println(Integer.toString(field.getRow()) + "," + Integer.toString(field.getColumn())); client2Settings.getBufferedWriter().flush();} catch (Exception Exception){Exception.printStackTrace();}});
